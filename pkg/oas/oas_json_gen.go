...
var jsonFieldsNameOfExtraCurrency = [4]string{
	0: "id",
	1: "amount",
	2: "name",
	3: "decimals",
...
func (s *Message) encodeFields(e *jx.Encoder) {
	...
	{
		if s.ExtraValue != nil {
			e.FieldStart("extra_value")
			e.ArrStart()
			for _, elem := range s.ExtraValue {
				elem.Encode(e)
			}
			e.ArrEnd()
		}
...
func (s *Message) Decode(d *jx.Decoder) error {
	...
	if err := d.ObjBytes(func(d *jx.Decoder, k []byte) error {
		...
		case "extra_value":
			if err := func() error {
				s.ExtraValue = make([]ExtraCurrency, 0)
				if err := d.Arr(func(d *jx.Decoder) error {
					var elem ExtraCurrency
					if err := elem.Decode(d); err != nil {
						return err
					}
					s.ExtraValue = append(s.ExtraValue, elem)
					return nil
				}); err != nil {
					return err
				}
				...
				return errors.Wrap(err, "decode field \"extra_value\"")
			}
		case "fwd_fee":
			requiredBitSet[0] |= 1 << 7
			if err := func() error {
